...
%'pml_thickness' enclosed both in x and y.
...
design_region = [2.4;2.4]; %% [x-length;y-length]
...
[rigeig,rigeigval] = ModeSolverTM(h,dim,BC,epsilon,num_mode,omega);

figure;
subplot(2,2,1);imagesc(real(reshape(rigeig(1:N*M,1),N,M).'));
subplot(2,2,2);imagesc(imag(reshape(rigeig(1:N*M,1),N,M).'));
subplot(2,2,3);imagesc(real(reshape(rigeig(1+N*M:2*N*M,1),N,M).'));
subplot(2,2,4);imagesc(imag(reshape(rigeig(1+N*M:2*N*M,1),N,M).'));
