...
function [rigeig,rigeigval] = ModeSolverTM(h,dim,BC,epsilon,num_mode,omega)
    ...
    [Maxwell] = Maxwell_Operator_Construct(dim, h, BC, (Matx .* pmlx).', (Maty .* pmly).', (Matz .* pmlz).');
    B = spdiags([reshape(epsilon',N*M,1);reshape(epsilon',N*M,1)],0,N*M*2,N*M*2);
    A = sparse(Maxwell);
